...
public class VirtuosoServiceImpl implements VirtuosoService {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      String query =
          "SELECT ?subject ?predicate ?object WHERE { GRAPH <"
              + graphName
  ...
  public Ontology getOntology(String graphName)
  ...
  @Override
  public JSONObject querySparql(String query)
      throws IOException,
          URISyntaxException,
    ...
    do {
      ...
      offset += limit;
      // URI endpoint = new URI(appConfig.getVirtuosoEndpoint() + "?format=json&query=" +
      // URLEncoder.encode(query2, StandardCharsets.UTF_8));
      URI endpoint =
          new URI(
              endPoint + "?format=json&query=" + URLEncoder.encode(query2, StandardCharsets.UTF_8));
      ...
      if (authResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
        ...
        BufferedReader rd =
            new BufferedReader(new InputStreamReader(authResponse.getEntity().getContent()));
        ...
        if (authResponse.containsHeader("WWW-Authenticate")) {
          ...
          final Header solution =
              md5Auth.authenticate(
                  // new UsernamePasswordCredentials(appConfig.getVirtuosoUser(),
                  // appConfig.getVirtuosoPassword()),
                  new UsernamePasswordCredentials(login, password),
                  new BasicHttpRequest(HttpGet.METHOD_NAME, endpoint.getPath()),
          ...
          throw new Error(
              "Web-service responded with Http 401, "
                  + "but didn't send us a usable WWW-Authenticate header.");
        }
      } else {
        throw new Error(
            "Didn't get an Http 401 "
                + "like we were expecting, instead we got:"
  ...
  @Override
  public Ontology readOntologyFromVirtuoso(String graphName, boolean bringCreatedAxioms)
      throws OWLOntologyCreationException, NoVirtuosoTriplesException {
    ...
    try {
      ...
      if (bringCreatedAxioms) {
        ...
        for (int i = 0; i < graphNames.length(); i++) {
          String matchedGraphName =
              graphNames.getJSONObject(i).getJSONObject("graph").getString("value");
    ...
    for (int i = 0; i < triples.length(); i++) {
      ...
      } else if (subjectType.equals("bnode")
          || subjectValue.startsWith("nodeID://")
      ...
      } else if (objectType.equals("bnode")
          || objectValue.startsWith("nodeID://")
      ...
      o.asGraphModel()
          .add(
              ResourceFactory.createStatement(
                  subject, ResourceFactory.createProperty(predicate), object));
    }
  ...
  public JSONArray getTriplesVirtuosoSparql(String graphName)
    ...
    String query =
        "SELECT ?subject ?predicate ?object WHERE { GRAPH <"
            + graphName
            + "> { ?subject ?predicate ?object . FILTER(isIRI(?object) || isBlank(?object)) } }";
    // String query = "SELECT ?subject ?predicate ?object WHERE {?subject ?predicate ?object} LIMIT
    // 10";
  ...
  @Override
  public boolean saveTriples(
      String graphName, String classUri, String axiomType, ArrayList<jenaTripleParser> triplesList)
    ...
    for (jenaTripleParser triple : triplesList) {
      triples.add(
          new jenaTripleParser(triple.getSubject(), triple.getPredicate(), triple.getObject()));
    }
    ...
    //
    //
    //            newGraphName = graphName + "concepts/" + classLastPart + "/" + axiomType + "/" +
    // uuid + "/";
    //            //create new graph
    ...
    for (jenaTripleParser triple : triples) {
      ...
      queryBuilder.append("<").append(triple.getSubject()).append("> ");

      // Handle the predicate (always a URI)
      queryBuilder.append("<").append(triple.getPredicate()).append("> ");

      queryBuilder.append("<").append(triple.getObject()).append("> ");
  ...
  public ArrayList<jenaTripleParser> replaceBlankNodesWithURIs(
      ArrayList<jenaTripleParser> triples) {
    for (jenaTripleParser triple : triples) {
  ...
  private JSONArray getGraphNamesVirtuosoSparql(String baseGraphName)
      throws IOException, URISyntaxException, AuthenticationException, MalformedChallengeException {
...
//        (406c8018-86e9-4ca2-b498-9290f3d18a4e <http://www.w3.org/1999/02/22-rdf-syntax-ns#first>
// <https://spec.industrialontologies.org/ontology/core/Core/ObjectiveSpecification>)
//        (406c8018-86e9-4ca2-b498-9290f3d18a4e <http://www.w3.org/1999/02/22-rdf-syntax-ns#rest>
// 3367f5cb-b12d-456d-94a9-1d52e5354d14)
//        (6369a3c4-a6b2-474d-a406-421f0792b1b9 <http://www.w3.org/2002/07/owl#intersectionOf>
// 406c8018-86e9-4ca2-b498-9290f3d18a4e)
//        (<https://spec.industrialontologies.org/ontology/core/Core/BusinessProcess>
// <http://www.w3.org/2000/01/rdf-schema#subClassOf> 6369a3c4-a6b2-474d-a406-421f0792b1b9)
//        (3367f5cb-b12d-456d-94a9-1d52e5354d14 <http://www.w3.org/1999/02/22-rdf-syntax-ns#first>
// <https://spec.industrialontologies.org/ontology/core/Core/PlanSpecification>)
//        (3367f5cb-b12d-456d-94a9-1d52e5354d14 <http://www.w3.org/1999/02/22-rdf-syntax-ns#rest>
