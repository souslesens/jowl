...
public class AxiomsController {
  ...
  @Autowired AxiomsService axiomsService;

  @Autowired VirtuosoService serviceVirtuoso;
  ...
  public ResponseEntity<String> manchesterAxiomConsistencyCheck(
      @RequestBody ManchesterToTriplesInput request) {
    ...
    try {
      ...
      return axiomsService.checkManchesterAxiomConsistency(graphName, axiom)
          ? ResponseEntity.ok().body("true")
          : ResponseEntity.status(422).body("false");
  ...
  public ResponseEntity<String> triples2manchester(@RequestBody TriplesToManchesterInput request) {
    ...
    if ((graphName == null || graphName.isEmpty() || triples == null || triples.length == 0)
      ...
      return ResponseEntity.badRequest()
          .body("Either (graphName and triples) or axiomGraphName must be provided.");
    }
  ...
  public ResponseEntity<String> getClassAxioms(@RequestBody GetClassAxiomsInput request) {
    ...
    if (graphName == null) {
      ...
      return ResponseEntity.status(400)
          .body("at least one format should be specified manchester format or triples");
    } else if (!(axiomType.isEmpty()
        || axiomType.equals("subclassof")
        || axiomType.equals("equivalentclass")
        || axiomType.equals("disjointwith"))) {
      return ResponseEntity.status(400)
          .body(
              "axiom type should be speicfied as one the follwoing: subclassof, equivalentclass, disjointwith  or be leaved empty for all");
    }

    try {
      String result =
          axiomsService.getClassAxioms(
              graphName, classUri, axiomType, manchesterFormat, triplesFormat);
  ...
  public ResponseEntity<String> checkTriplesConsistency(
      @RequestBody CheckTriplesCosistencyInput request) {
    ...
    try {
      return axiomsService.checkTriplesConsistency(graphName, triples, false)
          ? ResponseEntity.ok().body("true")
          : ResponseEntity.status(422).body("false");
    } catch (OWLOntologyCreationException e) {
      e.printStackTrace();
      return ResponseEntity.badRequest().body("Error while reading ontology From Triple Store");
    } catch (NoVirtuosoTriplesException
  ...
  public ResponseEntity<String> saveTriples(@RequestBody CheckTriplesCosistencyInput request) {
    ...
    try {
      return axiomsService.checkTriplesConsistency(graphName, triples, true)
          ? ResponseEntity.ok().body("triples saved successfully")
          : ResponseEntity.status(422)
              .body("the triples are not consistent with the ontology, cannot save");
    } catch (OWLOntologyCreationException e) {
      e.printStackTrace();
      return ResponseEntity.badRequest().body("Error while reading ontology From Triple Store");
    } catch (NoVirtuosoTriplesException
  ...
  public ResponseEntity<String> listClassesWithAxioms(
      @RequestBody ListClassesWithAxiomsInput request) {
    String graphName = request.getGraphName();
    String axiomType = request.getAxiomType();
    ...
    if (graphName == null || graphName.isEmpty()) {
      return ResponseEntity.badRequest().body("Graph Name should be provided");
        ...
        || axiomType.equals("subclassof")
        || axiomType.equals("equivalentclasses")
        || axiomType.equals("disjointwith"))) {
      return ResponseEntity.status(400)
          .body(
              "axiom type should be speicfied as one the follwoing: subclassof, equivalentclasses, disjointwith  or be leaved empty for all");
    }

    try {
      return ResponseEntity.ok(
          axiomsService.listClassesWithAxioms(graphName, axiomType, complexAxioms).toString());
    } catch (OWLOntologyCreationException e) {
      e.printStackTrace();
      return ResponseEntity.status(500).body("Error while creating the ontology");
    } catch (NoVirtuosoTriplesException e) {
