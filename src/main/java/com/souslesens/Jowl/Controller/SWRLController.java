...
public class SWRLController {
  ...
  @Autowired private SWRLService SWRLService;

  // Post API For STRING
  @PostMapping("/alternative_exec_rule")
  public ResponseEntity<?> postReclassification(
      @RequestBody(required = false) ruleSWRLInput request) {
    ...
    try {
      ...
      if (!(filePath == null) || !(url == null)) {
        ...
        result =
            SWRLService.SWRLruleReclassificationB64(ontologyContentDecoded64, reqBodies, reqHead);
      }
  ...
  public ResponseEntity<?> postPropertyValueAssignment(
      @RequestBody(required = false) ruleSWRLInputComplex request) {
    String filePath = request.getFilePath();
    String url = request.getUrl();
    String ontologyContentEncoded64 = request.getOntologyContentEncoded64();
    byte[] ontologyContentDecoded64Bytes = null;
  ...
  // This to count the number of the parameteres that will help us to know how many parameteres we
  // passed to the API
