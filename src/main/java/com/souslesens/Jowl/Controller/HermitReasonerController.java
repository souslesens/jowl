...
public class HermitReasonerController {
  ...
  @Autowired HermitReasonerService hermitReasonerService;

  @GetMapping(value = "/test")
  public ResponseEntity<String> pong() {

    return new ResponseEntity<String>(
        "Test du serveur Spring: " + HttpStatus.OK.name(), HttpStatus.OK);
  }
  ...
  public ResponseEntity<String> getConsistency(
    ...
    try {
      return new ResponseEntity<String>(
          hermitReasonerService.getConsistency(filePath, url, graphName), HttpStatus.OK);
    } catch (Exception e) {
  ...
  public ResponseEntity<String> getUnsatisfiableClasses(
    ...
    try {
      return new ResponseEntity<String>(
          hermitReasonerService.getUnsatisfaisableClasses(filePath, url, graphName), HttpStatus.OK);
    } catch (Exception e) {
      return new ResponseEntity<String>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  public ResponseEntity<String> getInferences(
    ...
    try {
      return new ResponseEntity<String>(
          hermitReasonerService.getInferences(
              filePath, url, Collections.singletonList("All"), graphName),
          HttpStatus.OK);
    } catch (Exception e) {
      return new ResponseEntity<String>(e.getMessage(), HttpStatus.INTERNAL_SERVER_ERROR);
    }
  ...
  public ResponseEntity<?> postConsistency(@RequestBody(required = false) reasonerInput request) {
    ...
    int parametersCount =
        countNumberOfParametres(ontologyContentDecoded64, filePath, url, graphName);
    if (parametersCount == 0) {
      return ResponseEntity.badRequest().body("At least one of params should be provided");
    } else if (parametersCount > 1) {
      return ResponseEntity.badRequest().body("Only one of params should be provided");
  ...
  public ResponseEntity<?> postUnsatisfiableClasses(
      @RequestBody(required = false) reasonerInput request) {
    // extract input parameters from the request object
    String filePath = request.getFilePath();
    String url = request.getUrl();
    String ontologyContentEncoded64 = request.getOntologyContentEncoded64();
  ...
  public ResponseEntity<?> postInferences(
      @RequestBody(required = false) reasonerInference request) {
    // extract input parameters from the request object
    String filePath = request.getFilePath();
    String url = request.getUrl();
    String ontologyContentEncoded64 = request.getOntologyContentEncoded64();
  ...
  public ResponseEntity<?> retrieveParameteresInference(
      @RequestBody(required = false) parametresInputInference request) {
    ...
    try {
      ...
      parametresInference.add(
          new parametresInputInference(
              equivalentClass,
              sameIndividual,
              IntersectionOf,
              UnionOf,
